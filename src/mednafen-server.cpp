...
struct ClientEntry
{
	...
	int64 incoming_tcp_pref = 0;
	int lag_frame = -1;
	uint32 counter  = 0;
	int prev_send_frame = 0;
...
int64 start_loop_time  = 0;
int64 get_buttons_time = 0;
int64 send_buttons_time = 0;

int64 pstart_loop_time  = 0;
int64 pget_buttons_time = 0;
int64 psend_buttons_time = 0;
int64 psleep_time = 0;
int64 psleep_amount = 0;
  struct sockaddr_in sockinu;
int main(int argc, char *argv[])
{
 ...
 for(unsigned fti = 0; fti < 2; fti++)
 {
  ...
  else
  {
   ...
   memset(&sockinu, 0, sizeof(sockinu));
   sockinu.sin_addr.s_addr = INADDR_ANY;
   sockinu.sin_family = AF_INET;
   sockinu.sin_port = htons(ServerConfig.Port+1);
  }
 ...
 // int 
  tcpopt = 1;
  if(setsockopt(lsd.fd, SOL_TCP, TCP_QUICKACK, &tcpopt, sizeof(int)))
  {
 //  ErrnoHolder ene(errno);

   //printf("Enabling option TCP_QUICKACK failed: %s\n", ene.StrError());
  // exit(-1);
  }
  ...
  serAddr.sin_port = htons(ServerConfig.Port+1);
...
memset(&serAddr, 0 , sizeof(serAddr));
  ...
  if((serSockDes = socket(AF_INET, SOCK_DGRAM, IPPROTO_IP)) < 0) {
  ...
  serAddr.sin_family = AF_INET;
  serAddr.sin_port = htons(ServerConfig.Port);
...
//	if(main_socket.initialize(0)==0)
//		printf("Socket initialized\n");
//	else { 
//		printf("Eror initialize socket\n");
//		return 1;
//	}
		...
		{
			    unsigned long ul = 1;
                ioctl(main_udp, FIONBIO, &ul);
		}
		socklen_t sad_ = sizeof(serAddr);
		if(bind(main_udp,(const struct sockaddr *)&serAddr,sad_)!=0)
		{
			printf("binding failed\n");
		}

//	main_udp_send = socket(AF_INET, SOCK_DGRAM, IPPROTO_IP);
//	if(main_udp_send != SOCKET_ERROR)
//	{
//		    unsigned long ul = 1;
//            ioctl(main_udp_send, FIONBIO, &ul);
//	}
...
start_loop_time = MBL_Time64();
  ...
  if(ListenSockets[lsi].family == AF_INET6)
       inet_ntop(ListenSockets[lsi].family, &sockin6.sin6_addr,  AllClients[n].ip, sizeof( AllClients[n].ip));
      else
	  {
       inet_ntop(ListenSockets[lsi].family, &sockin.sin_addr, AllClients[n].ip, sizeof(AllClients[n].ip));
	    memset(&AllClients[n].addrc, 0, 16);
		
	//	string(inet_ntoa(**(in_addr**)hostname->h_addr_list));
		AllClients[n].addrc.sin_addr = sockin.sin_addr;
		AllClients[n].addrc.sin_family    = AF_INET; // IPv4
        AllClients[n].addrc.sin_port = htons(ServerConfig.Port+1);
		/*
		AllClients[n].sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_IP);
		int one = 1;
		int zoo = setsockopt(AllClients[n].sock, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(one));
		if(AllClients[n].sock != SOCKET_ERROR)
		{
			    unsigned long ul = 1;
                ioctl(AllClients[n].sock, FIONBIO, &ul);
			
			
			
		}
		
		socklen_t sad_ = sizeof(sockinu);
				if(bind(AllClients[n].sock,(const struct sockaddr *)&sockinu,sad_)!=0)
		{
			printf("binding failed\n");
		}		
*/		
	//	 fcntl(AllClients[n].sock, F_SETFL, fcntl(AllClients[n].sock, F_GETFL) | O_NONBLOCK);
	//	 socklen_t  ah  = sizeof(AllClients[n].addrc);
	//	if(connect(AllClients[n].sock,(const struct sockaddr *)&AllClients[n].addrc,ah)!=0)
	//	{
 ...
 int received_bytes;
 uint32 countex = 0;
 #define MAX_UDP_PACKET 8+4+1
 do
 {
  sockaddr_in from;
  socklen_t fromLength = sizeof( from );
 // received_bytes = recvfrom( main_udp, (char*)lenght_buf, 2, 0, (sockaddr*)&from, &fromLength );
  

  received_bytes = recvfrom(main_udp, (char*)databuf, MAX_UDP_PACKET, 0, (sockaddr*)&from, &fromLength);
  if(received_bytes<1)
  {
	  if(errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR)
		continue;
	  printf("error: %s\n", ErrnoHolder(errno).StrError());
		continue;
  }
 if(received_bytes>0)
	for(int n = 0; n <  ServerConfig.MaxClients; n++)			//seek address what we got from recvfrom()
	{
	   if(AllClients[n].addrc.sin_addr.s_addr==from.sin_addr.s_addr)		//gotcha
	   {
			...
			{
				AllClients[n].is_udp  =1;
				AllClients[n].addrl =  from;
			//	AllClients[n].input_got = 20;
				AllClients[n].lag_frame=AllClients[n].frame_con; //latency is
				printf("Latency is %d\n", AllClients[n].lag_frame);
			}
		}
  ...
  for(int whichgame = 0; whichgame < ServerConfig.MaxClients; whichgame++)
  {
   ...
   //printf("%lld\n", last_time_inc);
Games[whichgame].clients_are_ready = 1;
...
//	main_socket.set_address( client->ip, ServerConfig.Port+1);
socklen_t sizeV = sizeof(client->addrl);
	int rv = recvfrom(client->sock, (char*)client->buf_client, 4+client->local_controllers_data_size+1, 0, (sockaddr*)&client->addrl, &sizeV);
//if(client->prev_send_frame)
	
uint32 countex = 0;
	if((rv>0))
	{
	
	memcpy(&countex,client->buf_client, 4);
	//counter++;
	//if((client->counter+client->lag_frame)>=client->frame_con)
	if(countex>client->counter)
	{
	memcpy(client->local_controller_buffer, client->buf_client+5, client->local_controllers_data_size);
	client->prev_send_frame = 1;
	client->counter = countex;
	}
//	 inet_ntop(AF_INET, &client->addrc.sin_addr, str_test, sizeof(str_test));
//	printf("buttons replaced: %d, %d, %s, %d\n", client->counter, client->frame_con, str_test, client->lag_frame);
	if(client->lag_frame==-1)
	{
	//	socklen_t sad_ = sizeof(client->addrl);
	//	client->addrl.sin_family = AF_INET;
	//	client->addrl.sin_port = client->addrc.sin_port;
	//	client->addrl.sin_addr.s_addr = inet_addr("0.0.0.0");
	//	
	//	if(bind(client->sock,(const struct sockaddr *)&client->addrl,sad_)!=0)
	//	{
	//		printf("binding failed\n");
	//	}	
		client->lag_frame=client->frame_con; //latency is
		printf("Latency is %d\n", client->lag_frame);
	}
	}
...
*/
	...
	//printf("last: %lld, prev: %lld, ready: %x, allready: %x\n", client->last_receive_time, client->prev_receive_time, client->ready, Games[whichgame].clients_are_ready);
	if(!client->input_got)
	{
...
//	if(client->input_got)printf("Inputs_got: %d, rdy: %d\n", client->input_got, Games[whichgame].clients_are_ready);
	...
	//client->prev_receive_time = client->last_receive_time;
   } // A games clients
//get_buttons_time = MBL_Time64();
   //
   //
   //
...
//   printf("cmd ?? %d\n", Games[whichgame].cmdSend);
  ...
  for(int whichgame = 0; whichgame < ServerConfig.MaxClients; whichgame ++)
  {
   ...
   if(sandwich <= 0)
   {
	 //  printf("something go wrong ct: %lld, gt: %lld, fps: %lld\n", curgametime, Games[whichgame].last_time, last_time_inc);
    sleep_amount = 0;
    break;
   }
  ...
  // curgametime = MBL_Time64();
  
 int64 timers = MBL_Time64();
  if(sleep_amount > 1000)
 {
//	  tv.tv_sec = 0;
//	  tv.tv_usec = sleep_amount;
//do
//{
...
while(sleep_amount>(timers-curgametime));
*/
...
//  psend_buttons_time =  send_buttons_time;
// psleep_amount = sleep_amount;
// psleep_time = MBL_Time64()-psend_buttons_time;
//printf("overtime: %lld, %lld\n", sleep_amount, sleeptime);
  //printf("%lld, %lld\n", sleep_amount, MBL_Time64() - curgametime);
 } // while(1)
}
