...
public:
    ...
    virtual int initialize(int param_port){
        port = param_port;
	socklen_t param_port2 = param_port;
        ...
        if (sock != SOCKET_ERROR) {
			...
			if(setsockopt(sock, SOL_SOCKET, SO_SNDBUF, &sndbufsize, sizeof(int)))
			{
			// Not a fatal error.
			...
			if(setsockopt(sock, SOL_SOCKET, SO_RCVBUF, &sndbufsize, sizeof(int)))
			{
			// Not a fatal error.
			ErrnoHolder ene(errno);
			
			printf("Warning: Rec buffer size set failed: %s", ene.StrError());
			}
          ...
          if (bind(sock, (sockaddr*)&tempaddr, sizeof(tempaddr))==0) 
			{
                ...
                if (port == 0) {
                    ...
                    getsockname(sock, (sockaddr*)&tempaddr, &param_port2);
    ...
    virtual int check_recv (char* buf, int len, sockaddr_in aadd)  {
        socklen_t V4 = sizeof(aadd);
        has_data_waiting = 0;
        int  lenn = 0;
        if ((lenn = recvfrom(sock, buf, len, 0, (sockaddr*)&aadd, &V4)) <= 0) {
            return 1;
        } else {
            return 0;
        }
